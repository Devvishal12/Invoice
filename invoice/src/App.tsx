...
function calcTotals(
  ...
  const subtotal = items.reduce(
    (sum, item) => sum + (Number.parseFloat(item.price as any) || 0) * (Number.parseInt(item.quantity as any) || 1),
    0
  );
...
function App() {
  ...
  useEffect(() => {
    ...
    setTitle(initialData.title); // Fixed typo: Changed href to title
  ...
  return (
    <ErrorBoundary>
      <div
        className="min-h-screen flex flex-col items-center py-4 sm:py-6 font-sans bg-[#fffbe9] dark:bg-gray-900 transition-colors duration-300"
      >
        {/* Navbar */}
        <nav
          className="flex flex-col sm:flex-row sm:justify-between sm:items-center w-full max-w-5xl py-2 px-2 sm:px-4 rounded-2xl border border-black/30 shadow-sm bg-white dark:bg-gray-800 dark:border-gray-700 gap-2 sm:gap-0"
        >
          <div className="flex items-center gap-2 self-center sm:self-auto">
            ...
            <span className="font-bold tracking-wide text-md text-black dark:text-white">
              InvoiceApp
            </span>
          </div>
        ...
        <section
          className="mt-6 sm:mt-8 w-full max-w-5xl rounded-3xl border-2 border-black/20 bg-[#eee3d2] dark:bg-gray-800 dark:border-gray-700 relative overflow-visible flex flex-col md:flex-row p-4 sm:p-6 md:p-8 gap-4 md:gap-8 shadow-[3px_3px_0_0_#cfbfa5] dark:shadow-[3px_3px_0_0_#4b5563]"
        >
          {/* Dimensional file sticker */}
          <div
            className="absolute -left-3 sm:-left-7 top-2 sm:top-4 bg-[#fff] border-2 border-black px-3 sm:px-4 py-1 rounded-lg rotate-[-8deg] shadow-md text-xs font-bold text-[#cb60b6] dark:bg-gray-700 dark:text-[#d78bca] dark:border-gray-600 select-none z-10"
          >
            dimensional '13 file
          </div>
          {/* Left: Invoice form */}
              ...
              <div className="flex flex-col gap-3 mt-2">
                {items.map((item, idx) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      className="text-[#ee6969] test-xs font-bold ml-2 hover:underline dark:text-[#f87171] hover:scale-105 transition-transform"
          ...
          <div className="flex-1 flex items-center justify-center relative min-h-[250px] md:min-h-[320px] mt-4 md:mt-0 w-full">
            <div
              className="w-full sm:w-[340px] md:w-[350px] bg-white dark:bg-gray-800 border-2 border-black/10 dark:border-gray-700 rounded-2xl px-4 sm:px-7 py-4 sm:py-8 drop-shadow-md flex flex-col gap-3 invoice-preview"
            >
              <InvoicePreview
                items={items}
                tax={tax}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-5">
            ...
            >
              ...
              <div className="text-xs text-gray-600 dark:text-gray-300">
                Fully flexible layout and fields
              </div>
            </div>
            <div
            ...
            >
              ...
              <div className="text-xs text-gray-600 dark:text-gray-300">
                Handle tax & discounts instantly
              </div>
            </div>
            <div
