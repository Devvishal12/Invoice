...
export default function InvoicePreview({
  ...
  return (
    <div className="bg-white border-2 border-black/20 rounded-xl w-[320px] text-black px-6 py-6 drop-shadow-md flex flex-col gap-2">
      <div className="font-bold text-lg mb-2 text-[#cb60b6]">Invoice Preview</div>
        ...
        <tbody>
          {items.map((item, idx) => (
            <tr key={idx} className="odd:bg-[#f8f3e7]">
              ...
              <td className="text-right">${Number.parseFloat(String(item.price)).toFixed(2)}</td>
              <td className="text-right">${((Number.parseFloat(String(item.price)) || 0) * (Number.parseInt(String(item.quantity)) || 1)).toFixed(2)}</td>
            </tr>
          ))}
        </tbody>
      </table>
